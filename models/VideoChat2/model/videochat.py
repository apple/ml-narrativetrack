...
from transformers import LlamaTokenizer, LlamaConfig, BitsAndBytesConfig

logger = logging.getLogger(__name__)


class VideoChat2Model(Blip2Base):
    """
    VideoChat2 model.
    Adapted from https://github.com/OpenGVLab/Ask-Anything/blob/main/video_chat2/models/videochat2_it.py and 
    https://github.com/OpenGVLab/Ask-Anything/blob/main/video_chat2/models/videochat2_pt.py
    """
    def __init__(self, config):
        ...
        if freeze_qformer:
            logger.info("Freezing QFormer weights...")
        ...
        logger.info('Loading LLaMA pre-trained model...')
        self.lm_tokenizer = LlamaTokenizer.from_pretrained(llama_model_path, use_fast=False)
        self.lm_tokenizer.pad_token = self.lm_tokenizer.eos_token
        logger.info(f'LLaMA pre-trained model from {llama_model_path} loaded.')
        ...
        if debug:
            logger.info("Entering debug mode, constructing small LLaMA model instead.")
            ...
            if self.low_resource:
                # NOTE: Use quantization config from BnB instead
        ...
        logger.info("Freezing LLaMA weights...")
        ...
        if videochat2_model_path:
            logger.info(f"Loading VideoChat2 from: {videochat2_model_path}")
    ...
    def encode_visual_features(self, image, instruction):
        device = image.device
        if self.low_resource:
            self.vit_to_cpu()
            image = image.to("cpu")
    ...
    def _get_text_len(self, text):
        return self.lm_tokenizer(text, return_tensors="pt", add_special_tokens=False).input_ids.shape[1]
    
    def forward_stage2(self, image, text_input):
        """
        Stage 2 pre-training for visual-language alignment with an LLM.
        """
        T = image.shape[1]
        use_image = True if T == 1 else False
        if self.prompt_list:
        ...
        img_embeds, atts_img = self.encode_visual_features(image)
        ...
        bos = torch.ones([batch_size, 1],
                         ...
                         device=to_regress_tokens.input_ids.device) * self.lm_tokenizer.bos_token_id
    ...
    def forward_stage3(self, image, text_input, instruction):
        """
        Stage 3 instruction-tuning.
        """
        img_embeds, use_image = self.encode_visual_features(image, instruction)
        ...
        for idx, prompt in enumerate(text_input):
            ...
            p_before_tokens = self.lm_tokenizer(p_before, return_tensors="pt", add_special_tokens=False).to(tmp_img_embeds.device)
            p_after_tokens = self.lm_tokenizer(p_after, return_tensors="pt", add_special_tokens=False).to(tmp_img_embeds.device)
            if self.use_lora:
                p_before_embeds = self.language_model.base_model.model.model.embed_tokens(p_before_tokens.input_ids)
                p_after_embeds = self.language_model.base_model.model.model.embed_tokens(p_after_tokens.input_ids)
            else:
                p_before_embeds = self.language_model.model.embed_tokens(p_before_tokens.input_ids)
                p_after_embeds = self.language_model.model.embed_tokens(p_after_tokens.input_ids)
        ...
        inputs_embeds = torch.ones([batch_size, txt_len], dtype=torch.long).to(img_embeds.device) * self.lm_tokenizer.pad_token_id
        if self.use_lora:
            inputs_embeds = self.language_model.base_model.model.model.embed_tokens(inputs_embeds)
        else:
            inputs_embeds = self.language_model.model.embed_tokens(inputs_embeds)
        ...
        # set bos_token
        inputs_embeds[:, :1] = self.lm_tokenizer.bos_token_id
    ...
    def forward(self, image, text_input, instruction=None):
        if instruction is None:
            return self.forward_stage2(image, text_input)
        else:
            return self.forward_stage3(image, text_input, instruction)
