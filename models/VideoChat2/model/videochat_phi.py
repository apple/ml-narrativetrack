...
class VideoChat2Phi3(Blip2Base):
    """
    VideoChat2 model with Phi3 base.
    Adapted from https://github.com/OpenGVLab/Ask-Anything/blob/main/video_chat2/models/videochat_phi/videochat2_it_phi.py
    """
    def __init__(self, config):
        ...
        logger.info('Loading Phi3 pre-trained model...')
        self.lm_tokenizer = AutoTokenizer.from_pretrained(phi_model_path, trust_remote_code=True)
        self.lm_tokenizer.padding_side = "left"
        if not self.lm_tokenizer.pad_token:
            self.lm_tokenizer.pad_token = self.lm_tokenizer.eos_token

        if self.debug:
            logger.info("Entering debug mode, constructing small Phi3 model instead.")
        ...
        logger.info("Freezing Phi3 weights...")
