...
class VideoChat2Mistral(Blip2Base):
    """
    VideoChat2 model with Mistral base.
    Adapted from https://github.com/OpenGVLab/Ask-Anything/blob/main/video_chat2/models/videochat_mistra/videochat2_it_mistral.py
    """
    def __init__(self, config):
        ...
        logger.info('Loading Mistral pre-trained model...')
        self.lm_tokenizer = AutoTokenizer.from_pretrained(mistral_model_path, use_fast=False)
        self.lm_tokenizer.padding_side = "left"
        if not self.lm_tokenizer.pad_token:
            self.lm_tokenizer.pad_token = self.lm_tokenizer.eos_token
            logger.info(f'LLaMA pre-trained model from {mistral_model_path} loaded.')

        if self.debug:
            logger.info("Entering debug mode, constructing small Mistral model instead.")
            ...
            self.language_model = AutoModelForCausalLM.from_config(mistral_config)
        else:
        ...
        logger.info("Freezing Mistral weights...")
    ...
    def forward(self, image, text_input, instruction):
        ...
        for idx, prompt in enumerate(text_input):
            ...
            if self.debug:  # Inspect and check the correctness of masking
                ...
                z = torch.where(z == -100, self.lm_tokenizer.unk_token_id, z)
                logger.info(self.lm_tokenizer.decode(z))
